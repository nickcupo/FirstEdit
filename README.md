...
how the code hangs together, [docs/DESIGN.md](docs/DESIGN.md) is what the app is
...
![The light table: one frame at comparison size, the whole burst underneath, Keep and Drop a hand apart](docs/images/lighttable.jpg)
...
![Compare: four near-identical frames of one exchange, each with its focus score](docs/images/compare-4.jpg)
...
measured. [docs/ML.md](docs/ML.md) has every number, every threshold and
...
[docs/WORKFLOW.md](docs/WORKFLOW.md) is the step-by-step from plugging the
...
(The short version. [docs/ML.md](docs/ML.md) has every number and its
origin.)
...
under it. Any face the global correction leaves more than half a stop from
target gets its own AI mask, prompted at the face's centre, the way
...
[docs/ROADMAP.md](docs/ROADMAP.md): the blink rule flags 654 of the 1,175 closed-eye CEW crops whose
...
- See DxO's output. Nothing here renders PhotoLab, so every number written
  ...
  eyedropper reading and 16 exports. Colour beyond white balance is left to
  DxO's rendering and to you.
