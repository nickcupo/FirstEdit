...
public struct ControlBar: View {
    ...
    @ViewBuilder
    private var cullStatus: some View {
        if model.showsSuggestion {
            CullSuggestionBadge()
                .help(model.cullLine + "\n" + Strings.LightTable.suggestionHelp)
        } else {
...
/// The same suggestion badge beside the photograph and in Full Image.
struct CullSuggestionBadge: View {
    var body: some View {
        Label(Strings.LightTable.suggested, systemImage: "star.fill")
            .font(.system(size: 11, weight: .bold))
            .foregroundStyle(.white)
            .padding(.horizontal, 6)
            .padding(.vertical, 2)
            .background(Tokens.Palette.suggestion, in: RoundedRectangle(cornerRadius: 3))
            .accessibilityLabel(Strings.LightTable.cullClearWin)
            .accessibilityHint(Strings.LightTable.suggestionHelp)
    }
}
