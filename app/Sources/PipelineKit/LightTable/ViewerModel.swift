...
public final class ViewerModel {
    ...
    /// A suggestion still standing, distinct from his explicit Keep or Drop.
    public var showsSuggestion: Bool {
        guard let row = currentRow else { return false }
        return row.rating >= VerdictValue.inThreshold && VerdictValue.his(row) == .unmarked
    }

    /// What the cull said about this frame, in its own words.
