...
/// * The cull's is an **outline**, top-right: a hollow circle for put forward,
///   a dotted one for set aside, an amber triangle for a fault it can name.
///   Never a tick, never a colour he uses.
...
final class FilmstripItemView: NSView {
    ...
    override func draw(_ dirty: NSRect) {
        ...
        let alpha: CGFloat = marks.fault != nil ? 0.5 : (marks.inStack && !marks.isStackTop ? 0.7 : 1)
        ctx.saveGState()
        ctx.setAlpha(alpha)
        ...
        // The cull's, outline only, top-right of the picture: white — or
        // amber for a fault it can name — on a dark disc. It was grey on the
        // photograph itself, and the dotted ring of "set aside" vanished on
        // every dark frame.
