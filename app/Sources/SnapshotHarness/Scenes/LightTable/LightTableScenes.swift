...
final class LightTableScenes: SceneProvider {
    override class var scenes: [SnapshotScene] {
        ...
        return [
            ...
            // A cull suggestion still standing, including its prominent badge.
            SnapshotScene(name: "lighttable-suggested", size: size) { f in
                let m = Demo.model(f)
                if let index = m.frames.firstIndex(where: { stem in
                    guard let row = m.session.rows[stem] else { return false }
                    return row.rating >= VerdictValue.inThreshold && VerdictValue.his(row) == .unmarked
                }) {
                    m.goToFrame(index)
                }
