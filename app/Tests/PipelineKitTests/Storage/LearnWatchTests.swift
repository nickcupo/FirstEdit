...
struct LearnWatch {
    ...
    func theNewestWatchIsTheOne() async throws {
        ...
        // The stub records /api/learned before load() resumes; the job title
        // arrives through a separate awaited /api/job request after that.
        // Wait for both observations, not a fixed slice of runner time.
        let deadline = ContinuousClock.now + .seconds(10)
        while (Self.reads(asked) <= before || model.inTheWay != "culling 2026-09-21"),
