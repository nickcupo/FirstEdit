...
4. **The machine and he never share a field.** The cull speaks in grey, hollow symbols and the words
   "the cull". His verdicts are filled badges and the words "you kept / you put out". Agreement is
...
| In Progress | shoot name | `photo.stack` | "54 frames · 14 kept" (his number, green); "Copy stopped at 412 of 1,558" when the copy's own log says it stopped — "412 frames" read like a whole night; "Copying the card…" while a copy into it is running or waiting on the list, with no count, since the row is only read again with the library | disclosure; expands when selected |
| ↳ | step rows | see §2.4 | current step shows "3/19" trailing, where it fits beside the whole step name (the name wins; the count stays in the row's help and VoiceOver value); before giving the count up the row closes the gap to 4 pt, because the selected row's heavier name at the 240 pt default otherwise lost the count on exactly the row he was on. A step the engine says is done has a `checkmark.circle.fill` in the same place, in the secondary colour, and "Done" in its help and VoiceOver value — every row looked the same done or not, so "what is left" meant opening steps; Choose Keepers counts until it is done, then has its check. Whether each is done is the library row's (§3.9-7), which is read again at every N, every job's end and on leaving a step, over the open session's, read when the shoot was opened | disabled steps say why on hover and in VoiceOver |
...
**All Shoots** is a `Table`, one row a shoot in the engine's order: Shoot · Up to · Frames · You kept ·
...
("Choose Keepers 140/288", the count drawn as the sidebar's is and given up before the name when the
...
the step and his count ("Presets · 14 you kept") — the native two-line title, which is where this
belongs. On Choose Keepers the bursts come first, with their unit, and the step's name is left out
("3 of 19 bursts · 14 you kept"; "been through" with no unit, beside "1 of 32 in burst 265", did not
...
none). The counts keep up with him: bursts looked through (here and the sidebar's "3/19") are the
...
> **23 frames will get a preset.** 14 you marked Keep · 9 the cull put forward in bursts you looked
> through and did not mark.  ☐ Leave those 9 out *(drawn only where the engine can be told to leave
> them out, §2.7 — no engine can yet, so today the line ends at "did not mark.")*
...
twice. "You marked Keep" rather than "you kept": the sidebar's "you kept" is that larger number.
...
- **The cull's:** top-right, **outline only** — `circle` put forward, `circle.dotted` set aside,
  `exclamationmark.triangle` (amber) a fault it can name, with the reason in the help tag. Never a
  tick, never a colour he uses. White (the fault amber) **on a 50 % black disc**: drawn in grey on
  the photograph itself, the dotted ring of "set aside" vanished on every dark frame. The
  hollow agreed marks sit on the same disc, for the same reason.
